...
.. tabs::
    ...
    .. group-tab:: AKS
       ...
       for instructions on how to install ``az`` and prepare your account. See
       notes below for more context on the operations.
       ...
       .. code-block:: bash
           ...
           # Delete the initial system node pool now that we have reconfigured
           # the cluster with properly tainted node pools.
       ...
       .. note::
          ...
          Node pools must be tainted with ``node.cilium.io/agent-not-ready=true:NoSchedule``
          to ensure that applications pods will only be scheduled once Cilium is
          ready to manage them, however on AKS:
          ...
          * It is not possible to assign custom node taints such as ``node.cilium.io/agent-not-ready=true:NoSchedule``
          ...
          these limitations, the operations above propose to:
